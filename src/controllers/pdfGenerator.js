...
async function formatActivityAsHTML(
  content,
  imageTitle = null,
  ...
  try {
    ...
    const activityContent =
      data.tasks || data.exercises || data.activity_components || data;
    ...
    const multiChoice =
      activityContent.multiple_choice_blanks ||
    ...
    if (openQ) {
      html +=
        '<div class="section"><h2>Open Question</h2><div class="open-question">';
...
async function generatePDF(req, res) {
  try {
    const { activityContent, activityId, imageTitle, customImageBase64 } =
      req.body;
    ...
    const htmlContent = await formatActivityAsHTML(
      activityContent,
      imageTitle,
      customImageBase64,
    );
